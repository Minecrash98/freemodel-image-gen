---
name: freemodel-image-gen
description: Call gpt-5.5 image generation through the freemodel.dev API. Use this skill when the user wants to generate images via the freemodel.dev free API, needs to set up image generation from a free endpoint, or asks about calling GPT image models without an OpenAI API key. Covers token setup, API format (Responses API + image_generation tool), streaming SSE parsing, reference images, size/quality constraints, and multi-token parallel generation.
---

# freemodel.dev Image Generation

Generate images via `https://api.freemodel.dev/v1/responses` using gpt-5.5 with the `image_generation` tool. No OpenAI API key required — only a free token.
...
Get a token from the API provider. Store it in an environment variable or `.env` file:

```
FREEMODEL_TOKEN=fe_oa_xxxxxxxx
```

Optionally add more tokens for parallel generation when `n > 1`:

```
FREEMODEL_TOKEN_2=fe_oa_yyyyyyyy
FREEMODEL_TOKEN_3=fe_oa_zzzzzzzz
```
...
Authorization: Bearer $FREEMODEL_TOKEN
...
```python
#!/usr/bin/env python3
"""freemodel_gen.py — Generate images via freemodel.dev API."""
import os, sys, json, base64, time
import requests
from pathlib import Path

FREEMODEL_URL = "https://api.freemodel.dev/v1/responses"

def load_tokens():
    tokens = []
    for key in ("FREEMODEL_TOKEN", "FREEMODEL_TOKEN_2", "FREEMODEL_TOKEN_3",
                "FREEMODEL_TOKEN_4", "FREEMODEL_TOKEN_5"):
        if os.environ.get(key):
            tokens.append(os.environ[key])
    if not tokens:
        raise SystemExit("Set FREEMODEL_TOKEN environment variable")
    return tokens
...
def generate_one(token, payload, call_index):
    for attempt in range(1, 4):
        try:
            r = requests.post(FREEMODEL_URL, json=payload,
...
if __name__ == "__main__":
    import argparse
    p = argparse.ArgumentParser(description="freemodel.dev image generation")

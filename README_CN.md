...
通过 [freemodel.dev](https://api.freemodel.dev) API 生成图片 — 无需 OpenAI Key。
...
# 3. 生成图片
python freemodel_gen.py "一颗放在白色桌子上的红苹果"
```

## 用法

```
python freemodel_gen.py "提示词" [选项]
...
```bash
# 基础用法
python freemodel_gen.py "日落山景"

# 高质量 2K
python freemodel_gen.py "未来城市" -q high -s 2048x2048

# 4 张并行生成 (配合多个 token)
python freemodel_gen.py "一只可爱的猫" -n 4

# 带参考图
python freemodel_gen.py "给这只猫画一顶帽子" -r cat.png -r style_ref.png
```
...
- 端点：`POST https://api.freemodel.dev/v1/responses`
